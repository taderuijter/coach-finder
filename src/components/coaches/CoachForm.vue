<template>
  <form @submit.prevent="submitForm">
    <div class="form-control" :class="{ invalid: !firstName.isValid }">
      ...
      <p v-if="!firstName.isValid">Firstname can't be empty.</p>
    </div>

    <div class="form-control" :class="{ invalid: !lastName.isValid }">
      <label for="lastname">Lastname</label>
      <input type="text" id="lastname" v-model.trim="lastName.val" @blur="clearValidation('lastName')" />
      <p v-if="!lastName.isValid">Lastname can't be empty.</p>
    </div>

    <div class="form-control" :class="{ invalid: !description.isValid }">
      <label for="description">Description</label>
      <textarea id="description" row="5" v-model.trim="description.val" @blur="clearValidation('description')"></textarea>
    ...
    <div class="form-control" :class="{ invalid: !areas.isValid }">
      ...
      <div>
        <input type="checkbox" id="freelance" value="freelance" v-model="areas.val" @blur="clearValidation('areas')" />
      ...
      <p v-if="!areas.isValid">At least one needs to be checked</p>
    </div>
    <p v-if="!formIsValid">Fix the fields and submit again.</p>
